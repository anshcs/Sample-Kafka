...
BROKER2_ADDR = (SERVER, 5051)


def send_metadata():
    nconnected = True
    broker_comm = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    while nconnected:
        try:
            broker_comm.connect(BROKER2_ADDR)
            nconnected = False
        except:
            pass
    topic = topicName[-1]
    b = 'b'
    b_length, id = send_encoding_msg(b)
    t_length, topic_msg = send_encoding_msg(topic)
    broker_comm.send(b_length)
    broker_comm.send(id)
    broker_comm.send(t_length)
    broker_comm.send(topic_msg)
